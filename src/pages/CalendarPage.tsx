...
import { parseISO, format, startOfMonth, endOfMonth, addMonths, eachDayOfInterval, isSameDay, addDays } from 'date-fns';
...
export default function CalendarPage() {
  ...
  const events = useMemo(() => {
    ...
    vehicles.forEach(v => {
      ...
      if (lastM) {
        evts.push({
          date: parseISO(lastM.nextInspectionDate),
          label: `Truck #${v.truckNumber} - Next Inspection`,
          type: 'maintenance',
        });
      }
      ...
      if (lastB) {
        const nextBrake = addDays(parseISO(lastB.testDate), 42);
        evts.push({
          date: nextBrake,
          label: `Truck #${v.truckNumber} - Brake Test Due`,
          type: 'brake',
        });
      }
    });
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Upcoming Inspections – {format(now, 'MMMM yyyy')}</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            {days.map(day => {
              ...
              return (
                <div key={day.toISOString()} className={cn(
                  "min-h-[60px] p-1 rounded-md border text-xs",
                  ...
                  {dayEvents.map((e, i) => (
                    <p key={i} className={cn(
                      "truncate text-[10px] mt-0.5 px-1 rounded",
                      e.type === 'brake' ? 'bg-destructive/20 text-destructive' : 'bg-primary/20 text-primary'
                    )}>
                      {e.label.split(' - ')[0]}
                    </p>
                  ))}
